...
function parsePath(route, currentPath) {
	var splitPath = currentPath.map(split);
	var keys = {};
...
function createRouter(currentPath, routes) {
	...
	function render() {
		return currentRoute.view(function(route) {
			...
			var params = typeof route.path === "string" && route.path.indexOf(":") !== -1
				? parsePath(route, currentPath)
				: null;
	...
	var currentRoute = currentPath.map(getRoute);
